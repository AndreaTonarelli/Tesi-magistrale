...
def gammaCoeff(gamma, hx,hy, nout_start,nout_end, XX):
    ...
    X9, X10, X11, X12, X13, X14, X15, X16, X17, X18, X19, X20, X21, X22, X23, X24 = XX[8:]

    gamma = gamma_corners_obst(gamma, X9,  hx, hy, 'up')     # Obstacle 9 (Look at the position!!!)
    ...
    gamma = gamma_corners_obst(gamma, X16, hx, hy, 'down')   # Obstacle 16
    gamma = gamma_corners_obst(gamma, X17, hx, hy, 'up')     # Obstacle 17
    gamma = gamma_corners_obst(gamma, X18, hx, hy, 'down')   # Obstacle 18
    gamma = gamma_corners_obst(gamma, X19, hx, hy, 'up')     # Obstacle 19
    gamma = gamma_corners_obst(gamma, X20, hx, hy, 'down')   # Obstacle 20
    gamma = gamma_corners_obst(gamma, X21, hx, hy, 'up')     # Obstacle 21
    gamma = gamma_corners_obst(gamma, X22, hx, hy, 'down')   # Obstacle 22

    xs23 = X23[0]; xe23 = X23[1]; ys23 = X23[2]; ye23 = X23[3]
    gamma[xs23-1, ys23] = hx*hy / (hx**2 + hy**2)
    gamma[xe23, ye23+1] = hx*hy / (hx**2 + hy**2)

    xs24 = X24[0]; xe24 = X24[1]; ys24 = X24[2]; ye24 = X24[3]
    gamma[xs24-1, ye24] = hx*hy / (hx**2 + hy**2)
    gamma[xe24, ys24-1] = hx*hy / (hx**2 + hy**2)
...
def PlotFunctions(xx, yy, phi, x, y, XX, Lx, Ly, title, x_label, y_label):
    ...
    cbar = plt.colorbar(plot, ax = ax, ticks = np.linspace(0, np.max(phi), 12))
    for i in range(len(XX)):
        add_obst_patch(XX[i])
