...
from   Plates_u_single_functions import *
from   Plates_u_single_obstacles_def import *
...
Time = 5*3600        # s     (simulation time)
method = 'Upwind'  # Discretization method (CDS or Upwind)
Plot_title = f'Conf: Plates - {u_in} m/s - {nx}x{ny} - {round(Time/3600,0)} h'
...
Xentrance = Obstacles(x, y, Lx, Ly, 0.25)[2]
ys_entrance = Xentrance[0]; ye_entrance = Xentrance[1]

# Inlet section (north side)
nin_start1 = ys_entrance # first cell index (pay attention to Python INDEX!!!)
nin_end1 = ye_entrance   # last cell index
...
Din = hy*(nin_end1-nin_start1+1)      # inlet section area [m]
Dout = hy*(nout_end-nout_start+1)     # outlet section area [m]
u_out = u_in* Din**2/Dout**2
...
for it in range(1, nsteps+1):
    ...
    Qin  = np.mean(u[0, nin_start1-1:nin_end1+1])*Din    # inlet flow rate [m3/s]
    Qout = np.mean(u[-1, nout_start-1:nout_end+1])*Dout  # outlet flow rate [m3/s]    
    if (abs(Qout)>1.e-6):
...
filename = f'Plates_{nx}x{ny}_{Time/3600}h_{u_in}m_s.txt'
dictionary = { 't': t_vector, 'conc_B1_out': cB1out, 'mass_B1_out': massB1out , 'Glc_max': cGlc_max, 'Gln_max': cGln_max , 'Amm_max': cAmm_max, 'Lac_max': cLac_max , 'O2_max': cO2_max, 'B1_max': cB1_max}
...
plt.savefig('1_Ammonia_conc.png')
...
# Mean values of cB
MeanCellsPlot(t_vector, [cBmean1, cBmean2, cBmean3, cBmean4, cBmean5, cBmean6, cBmean7, cBmean8], Time, Xv0, Plot_title)
