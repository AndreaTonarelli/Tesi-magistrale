...
def gammaCoeff(gamma, hx,hy, nout_start, nout_end, XX):
    ...
    def gamma_corners_obst(gamma, X, hx, hy, position):
        ...
        if position == 'down':
            ...
            gamma[xs-1, ye-1] = hx*hy / (hx**2 + hy**2)
            gamma[xe_last+1, ye-1] = hx*hy / (hx**2 + hy**2)
            ...
            for i in range((len(X) // 4)-1):
                ...
                gamma[xe1+1, ye2] = hx*hy / (hx**2 + hy**2)
            return gamma
        elif position == 'south':
            xs = X[0]; ys = X[2]; xe_last = X[-3]
            gamma[xs-1, ys] = hx*hy / (hx**2 + hy**2)
    ...
    X1, X2, X3, X4, X5, X6, X7, X8, X9, X10, X11, X12, X13, X14, X15, X16, X17, X18, X19, X20, X21, X22, X23, X24, X25, X26, X27, X28, X29, X30, X31, X32, X33 = XX
    xs1 = X1[0]; ys1 = X1[2]; ye1 = X1[3]
    gamma[xs1, ys1-1] = hx*hy / (hx**2 + hy**2)              # Obstacle 1
    gamma[xs1, ye1+1] = hx*hy / (hx**2 + hy**2)
    gamma = gamma_corners_obst(gamma, X2, hx, hy, 'down')    # Obstacle 2
    gamma = gamma_corners_obst(gamma, X3, hx, hy, 'up')      # Obstacle 3
    gamma = gamma_corners_obst(gamma, X4, hx, hy, 'down')    # Obstacle 4
    gamma = gamma_corners_obst(gamma, X5, hx, hy, 'up')      # Obstacle 5
    gamma = gamma_corners_obst(gamma, X6, hx, hy, 'down')    # Obstacle 6
    gamma = gamma_corners_obst(gamma, X7, hx, hy, 'up')      # Obstacle 7
    gamma = gamma_corners_obst(gamma, X8, hx, hy, 'down')    # Obstacle 8
    gamma = gamma_corners_obst(gamma, X9, hx, hy, 'up')      # Obstacle 9
    gamma = gamma_corners_obst(gamma, X10, hx, hy, 'down')   # Obstacle 10
    gamma = gamma_corners_obst(gamma, X11, hx, hy, 'up')     # Obstacle 11
    gamma = gamma_corners_obst(gamma, X12, hx, hy, 'down')   # Obstacle 12
    gamma = gamma_corners_obst(gamma, X13, hx, hy, 'up')     # Obstacle 13
    gamma = gamma_corners_obst(gamma, X14, hx, hy, 'down')   # Obstacle 14
    gamma = gamma_corners_obst(gamma, X15, hx, hy, 'up')     # Obstacle 15
    xs16 = X16[0]; ye16 = X16[3]
    gamma[xs16-1, ye16-1] = hx*hy / (hx**2 + hy**2)          # Obstacle 16
    xs17 = X17[0]; ys17 = X17[2]
    gamma[xs17-1, ys17] = hx*hy / (hx**2 + hy**2)            # Obstacle 17
    
    gamma = gamma_corners_obst(gamma, X18, hx, hy, 'south')   # Obstacle 18 (Look at the position!!!)
    gamma = gamma_corners_obst(gamma, X19, hx, hy, 'north')   # Obstacle 19
    gamma = gamma_corners_obst(gamma, X20, hx, hy, 'south')   # Obstacle 20
    gamma = gamma_corners_obst(gamma, X21, hx, hy, 'north')   # Obstacle 21
    gamma = gamma_corners_obst(gamma, X22, hx, hy, 'south')   # Obstacle 22
    gamma = gamma_corners_obst(gamma, X23, hx, hy, 'north')   # Obstacle 23
    gamma = gamma_corners_obst(gamma, X24, hx, hy, 'south')   # Obstacle 24
    gamma = gamma_corners_obst(gamma, X25, hx, hy, 'north')   # Obstacle 25
    gamma = gamma_corners_obst(gamma, X26, hx, hy, 'south')   # Obstacle 26
    gamma = gamma_corners_obst(gamma, X27, hx, hy, 'north')   # Obstacle 27
    gamma = gamma_corners_obst(gamma, X28, hx, hy, 'south')   # Obstacle 28
    gamma = gamma_corners_obst(gamma, X29, hx, hy, 'north')   # Obstacle 29
    gamma = gamma_corners_obst(gamma, X30, hx, hy, 'south')   # Obstacle 30
    gamma = gamma_corners_obst(gamma, X31, hx, hy, 'north')   # Obstacle 31

    xs32 = X32[0]; xe32 = X32[1]; ys32 = X32[2]; ye32 = X32[3]
    gamma[xs32-1, ys32] = hx*hy / (hx**2 + hy**2)              # Obstacle 32
    xs33 = X33[0]; xe33 = X33[1]; ys33 = X33[2]; ye33 = X33[3]
    gamma[xs33-1, ye33] = hx*hy / (hx**2 + hy**2)              # Obstacle 33
    ...
    for i in range(1, nx+1):
        for j in range(1, ny+1):
            ...
            if Xv[i,j] != 0:
                ...
                cB1[i,j]  = cB1star[i,j] + dt*Q_B1*Xv[i,j]*(1 - mu/mu_max)
...
def B1OutletConcentration(cB1, nout_start, nout_end):
    ...
    for j in range(nout_start, nout_end+1):
        ...
        cB1out = np.append(cB1out, cB1[-1, j])
        
    cB1out_mean = np.mean(cB1out)
...
# Create a vector for cumulative outlet B1 concentration and mass
def cumulativeB1out(cB1out, t):
    
    cB1out_cumulative = np.empty(0)

    for i in range(1, len(cB1out)+1):
        cB1out_cumulative = np.append(cB1out_cumulative, np.trapz(cB1out[:i], t[:i]))
        
    return cB1out_cumulative



# Evaluating the total mass of protein inside the reactor
@njit
def totalmassB1_function(cB1, MW, hx, hy, nx, ny):

    massB1_single_cell = np.zeros_like(cB1)

    for i in range(nx+2):
        for j in range(ny+2):
            if cB1[i,j] != 0:

                massB1_single_cell[i,j] = cB1[i,j]*hx**2*hy*MW

    massB1_total = np.sum(massB1_single_cell)

    return massB1_total



# Evaluating the total mass of protein inside the reactor (2nd method)
@njit
def totalmassB1_function_2(cB1, Xv0, MW_B1):

    Area = 0.8*16                                         # [m2]
    Cell_diameter = 1.5e-5                                # [m]
    Cell_volume = 4/3 * np.pi * (Cell_diameter/2)**3      # [m3/cell]
    Volume_layer = Area * Cell_diameter                   # [m3]
    Total_cells = Volume_layer * Xv0                      # [cells]
    Total_cell_volume = Total_cells * Cell_volume         # [m3]
    cB1_total = np.sum(cB1)                               # [mol/m3]
    massB1_total = cB1_total * Total_cell_volume * MW_B1  # [g]
...
def MeanCellsPlot(t_vec, cBmean, Time, cB0, title):
    t_vector = t_vec/3600
    Time = Time/3600
    cBmax = 0
    plt.figure(figsize = (10, 10))
    for i in range(len(cBmean)):
        plt.plot(t_vector,cBmean[i], color = 'black')
        cBmax = max(cBmax, np.max(cBmean[i]))
    plt.title(title, fontsize = title_fontsize)
    plt.xlabel('Time [h]', fontsize = axis_fontsize)
    plt.ylabel('Mean number of cells on each plate [cells]', fontsize = axis_fontsize)
    plt.xticks(fontsize = number_fontsize)
    plt.yticks(fontsize = number_fontsize)
    plt.xlim(0,Time)
    plt.ylim(cB0, cBmax)
...
def PlotinTime(t_vec, phi, Time, title, y_label):
    ...
    Time = Time/3600
    plt.figure(figsize = (10, 10))
    plt.plot(t_vector, phi, linewidth = 2.0, color = 'black')
    plt.title(title, fontsize = title_fontsize)
    plt.xlabel('Time [h]', fontsize = axis_fontsize)
    plt.ylabel(y_label, fontsize = axis_fontsize)
    plt.xticks(fontsize = number_fontsize)
