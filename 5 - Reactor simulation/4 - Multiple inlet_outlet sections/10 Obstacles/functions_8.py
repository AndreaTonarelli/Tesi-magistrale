...
def gammaCoeff(gamma, hx,hy, nout_s,nout_e, X1, X2, X3, X4, X5, X6, X7, X8, X9, X10):

    nout_start1 = nout_s[0]; nout_start2 = nout_s[1]; nout_start3 = nout_s[2]; nout_start4 = nout_s[3]; nout_start5 = nout_s[4]
    nout_start6 = nout_s[5]; nout_start7 = nout_s[6]; nout_start8 = nout_s[7]; nout_start9 = nout_s[8]
    nout_end1   = nout_e[0]; nout_end2   = nout_e[1]; nout_end3   = nout_e[2]; nout_end4   = nout_e[3]; nout_end5   = nout_e[4]
    nout_end6   = nout_e[5]; nout_end7   = nout_e[6]; nout_end8   = nout_e[7]; nout_end9   = nout_e[8]
    ...
    gamma[-2, nout_start1-1:nout_end1-1] = hx*hy / (2*hx**2 + 2*hy**2)    # Outlet section is treated as an internal cell
    gamma[-2, nout_start1-1]             = hx*hy / (  hx**2 + 2*hy**2)    # Corners of outlet sections
    gamma[-2, nout_end1-1]               = hx*hy / (  hx**2 + 2*hy**2)
    gamma[nout_start2-1:nout_end2-1, -2] = hx*hy / (2*hx**2 + 2*hy**2)    # Outlet section is treated as an internal cell
    gamma[nout_start2-1, -2]             = hx*hy / (2*hx**2 + hy**2)      # Corners of outlet sections
    gamma[nout_start3-1:nout_end3-1,  1] = hx*hy / (2*hx**2 + 2*hy**2)    # Outlet section is treated as an internal cell
    gamma[nout_start3-1,  1]             = hx*hy / (2*hx**2 + hy**2)      # Corners of outlet sections
    gamma[nout_start4-1:nout_end4-1, -2] = hx*hy / (2*hx**2 + 2*hy**2)    # Outlet section is treated as an internal cell
    gamma[nout_start4-1, -2]             = hx*hy / (2*hx**2 + hy**2)      # Corners of outlet sections
    gamma[nout_start5-1:nout_end5-1,  1] = hx*hy / (2*hx**2 + 2*hy**2)    # Outlet section is treated as an internal cell
    gamma[nout_start5-1,  1]             = hx*hy / (2*hx**2 + hy**2)      # Corners of outlet sections
    gamma[nout_start6-1:nout_end6-1, -2] = hx*hy / (2*hx**2 + 2*hy**2)    # Outlet section is treated as an internal cell
    gamma[nout_start6-1, -2]             = hx*hy / (2*hx**2 + hy**2)      # Corners of outlet sections
    gamma[nout_start7-1:nout_end7-1,  1] = hx*hy / (2*hx**2 + 2*hy**2)    # Outlet section is treated as an internal cell
    gamma[nout_start7-1,  1]             = hx*hy / (2*hx**2 + hy**2)      # Corners of outlet sections
    gamma[nout_start8-1:nout_end8-1, -2] = hx*hy / (2*hx**2 + 2*hy**2)    # Outlet section is treated as an internal cell
    gamma[nout_start8-1, -2]             = hx*hy / (2*hx**2 + hy**2)      # Corners of outlet sections
    gamma[nout_start9-1:nout_end9-1,  1] = hx*hy / (2*hx**2 + 2*hy**2)    # Outlet section is treated as an internal cell
    gamma[nout_start9-1,  1]             = hx*hy / (2*hx**2 + hy**2)      # Corners of outlet sections
...
def SpeciesBCs(phi, nx,ny, X1, X2, X3, X4, X5, X6, X7, X8, X9, X10):
