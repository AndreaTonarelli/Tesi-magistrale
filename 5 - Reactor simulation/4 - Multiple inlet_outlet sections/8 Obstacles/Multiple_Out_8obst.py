...
nin_start2 = xe3 + 1   # first cell index (pay attention cause it is in MATLAB notation!)
nin_end2 = xs4 - 1      # last cell index (pay attention cause it is in MATLAB notation!)
# Inlet section (south side)
nin_start3 = xe1 + 1   # first cell index (pay attention cause it is in MATLAB notation!)
nin_end3 = xs2 - 1      # last cell index (pay attention cause it is in MATLAB notation!)
...
# Outlet section (north side)
nout_start2 = xe2 + 1                  # first cell index (pay attention cause it is in MATLAB notation!)
nout_end2 = int(xe2 + 0.25*(xs3-xe2))  # last cell index (pay attention cause it is in MATLAB notation!)
# Outlet section (south side)
nout_start3 = xe3 + 1                  # first cell index (pay attention cause it is in MATLAB notation!)
nout_end3 = int(xe3 + 0.25*(xs4-xe3))  # last cell index (pay attention cause it is in MATLAB notation!)
...
gamma = gammaCoeff(gamma, hx,hy, nout_start1,nout_end1, nout_start2,nout_end2, nout_start3,nout_end3, X1, X2, X3, X4, X5, X6, X7, X8)
...
for it in range(1, nsteps+1):
    ...
    u[0, nin_start1-1:nin_end1+1] = u_in   # fixed inlet velocity
    v[nin_start2-1:nin_end2+1, -1] = -v_in   # fixed inlet velocity
    v[nin_start3-1:nin_end3+1, 0] = v_in   # fixed inlet velocity
    ...
    u[nout_start2-1:nout_end2+1, -1] = u[nout_start2-1:nout_end2+1, -2]   # zero-gradient outlet velocity
    v[nout_start2-1:nout_end2+1, -1] = v[nout_start2-1:nout_end2+1, -2]   # zero-gradient outlet velocity
    u[nout_start3-1:nout_end3+1,  0] = u[nout_start3-1:nout_end3+1,  1]   # zero-gradient outlet velocity
    v[nout_start3-1:nout_end3+1,  0] = v[nout_start3-1:nout_end3+1,  1]   # zero-gradient outlet velocity
    ...
    ut[0, nin_start1-1:nin_end1+1] = u_in       # fixed inlet velocity
    vt[nin_start2-1:nin_end2+1, -1] = -v_in       # fixed inlet velocity
    vt[nin_start3-1:nin_end3+1, 0] = v_in       # fixed inlet velocity
    ...
    ut[nout_start2-1:nout_end2+1, -1] = u[nout_start2-1:nout_end2+1, -1]   # zero-gradient outlet velocity
    vt[nout_start2-1:nout_end2+1, -1] = v[nout_start2-1:nout_end2+1, -1]   # zero-gradient outlet velocity
    ut[nout_start3-1:nout_end3+1,  0] = u[nout_start3-1:nout_end3+1,  0]   # zero-gradient outlet velocity
    vt[nout_start3-1:nout_end3+1,  0] = v[nout_start3-1:nout_end3+1,  0]   # zero-gradient outlet velocity
    ...
    cO2[nin_start2-1:nin_end2+1, -1] = 2*cO2in - cO2[nin_start2-1:nin_end2+1, -2]
    cG[nin_start2-1:nin_end2+1, -1] = 2*cGin - cG[nin_start2-1:nin_end2+1, -2]
    cO2[nin_start3-1:nin_end3+1, 0] = 2*cO2in - cO2[nin_start3-1:nin_end3+1, 1]
    cG[nin_start3-1:nin_end3+1, 0] = 2*cGin - cG[nin_start3-1:nin_end3+1, 1]
...
# Mean values of cB
MeanCellsPlot(t_vector, cBmean1, cBmean2, cBmean3, cBmean4, cBmean5, cBmean6, cBmean7, Time, cB0*2)
