...
th = int(np.ceil(thick/hy))    # it finds the index not the positions!
...
nin_end = nx + 1     # last cell index (pay attention cause it is in MATLAB notation!)

# Outlet section (east side)
nout_start = 2       # first cell index (pay attention cause it is in MATLAB notation!)
nout_end = nx + 1    # last cell index (pay attention cause it is in MATLAB notation!)
...
gamma[nout_start-1:nout_end-1, -2] = hx*hy / (2*hx**2 + 2*hy**2)    # Outlet section is treated as an internal cell
gamma[nout_start-1, -2]            = hx*hy / (  hx**2 + 2*hy**2)    # Corners of outlet sections
gamma[nout_end-1, -2]              = hx*hy / (  hx**2 + 2*hy**2)
...
gamma[xe2, ys2-1] = hx*hy / (hx**2 + hy**2)          # Obstacle 2
gamma[xe2, ye2+1] = hx*hy / (hx**2 + hy**2)

gamma[xs3, ys3-1] = hx*hy / (hx**2 + hy**2)          # Obstacle 3
gamma[xs3, ye3+1] = hx*hy / (hx**2 + hy**2)

gamma[xe4, ys4-1] = hx*hy / (hx**2 + hy**2)          # Obstacle 4
gamma[xe4, ye4+1] = hx*hy / (hx**2 + hy**2)

gamma[xs5, ys5-1] = hx*hy / (hx**2 + hy**2)          # Obstacle 5
gamma[xs5, ye5+1] = hx*hy / (hx**2 + hy**2)

gamma[xe6, ys6-1] = hx*hy / (hx**2 + hy**2)          # Obstacle 6
gamma[xe6, ye6+1] = hx*hy / (hx**2 + hy**2)

gamma[xs7, ys7-1] = hx*hy / (hx**2 + hy**2)          # Obstacle 7
gamma[xs7, ye7+1] = hx*hy / (hx**2 + hy**2)

gamma[xe8, ys8-1] = hx*hy / (hx**2 + hy**2)          # Obstacle 8
gamma[xe8, ye8+1] = hx*hy / (hx**2 + hy**2)

gamma[xs9, ys9-1] = hx*hy / (hx**2 + hy**2)          # Obstacle 9
gamma[xs9, ye9+1] = hx*hy / (hx**2 + hy**2)
...
v[:, :] = v_in                     # Internal points: fixed velocity [m/s]
v[xs1:xe1+1, ys1-1:ye1+1] = 0      # Obstacle 1
v[xs2:xe2+1, ys2-1:ye2+1] = 0      # Obstacle 2
v[xs3:xe3+1, ys3-1:ye3+1] = 0      # Obstacle 3
v[xs4:xe4+1, ys4-1:ye4+1] = 0      # Obstacle 4
v[xs5:xe5+1, ys5-1:ye5+1] = 0      # Obstacle 5
v[xs6:xe6+1, ys6-1:ye6+1] = 0      # Obstacle 6
v[xs7:xe7+1, ys7-1:ye7+1] = 0      # Obstacle 7
v[xs8:xe8+1, ys8-1:ye8+1] = 0      # Obstacle 8
v[xs9:xe9+1, ys9-1:ye9+1] = 0      # Obstacle 9
vt = v

# Immobilized cells initialized over obstacles
cB[xs1:xe1, ye1+1:ye1+th+1] = cB0   # Obstacle 1
cB[xs2:xe2, ye2+1:ye2+th+1] = cB0   # Obstacle 2
cB[xs3:xe3, ye3+1:ye3+th+1] = cB0   # Obstacle 3
cB[xs4:xe4, ye4+1:ye4+th+1] = cB0   # Obstacle 4
cB[xs5:xe5, ye5+1:ye5+th+1] = cB0   # Obstacle 5
cB[xs6:xe6, ye6+1:ye6+th+1] = cB0   # Obstacle 6
cB[xs7:xe7, ye7+1:ye7+th+1] = cB0   # Obstacle 7
cB[xs8:xe8, ye8+1:ye8+th+1] = cB0   # Obstacle 8
cB[xs9:xe9, ye9+1:ye9+th+1] = cB0   # Obstacle 9
...
for it in range(1, nsteps+1):
    ...
    v[nin_start-1:nin_end+1, 0] = v_in   # fixed inlet velocity

    # Over-writing outlet conditions
    u[nout_start-1:nout_end+1, -1] = u[nout_start-1:nout_end+1, -2]   # zero-gradient outlet velocity
    v[nout_start-1:nout_end+1, -1] = v[nout_start-1:nout_end+1, -2]   # zero-gradient outlet velocity
    ...
    vt[nin_start-1:nin_end+1, 0] = v_in       # fixed inlet velocity
    ut[nout_start-1:nout_end+1, -1] = u[nout_start-1:nout_end+1, -1]   # zero-gradient outlet velocity
    vt[nout_start-1:nout_end+1, -1] = v[nout_start-1:nout_end+1, -1]   # zero-gradient outlet velocity
    ...
    # Inlet sections
    cO2[nin_start-1:nin_end+1, 0] = 2*cO2in - cO2[nin_start-1:nin_end+1, 1]
...
ccB[xs1:xe1, ye1+1:ye1+th+1] = cB[xs1+1:xe1+1, ye1+1:ye1+th+1]  # Obstacle 1
ccB[xs2:xe2, ye2+1:ye2+th+1] = cB[xs2+1:xe2+1, ye2+1:ye2+th+1]  # Obstacle 2
ccB[xs3:xe3, ye3+1:ye3+th+1] = cB[xs3+1:xe3+1, ye3+1:ye3+th+1]  # Obstacle 3
ccB[xs4:xe4, ye4+1:ye4+th+1] = cB[xs4+1:xe4+1, ye4+1:ye4+th+1]  # Obstacle 4
ccB[xs5:xe5, ye5+1:ye5+th+1] = cB[xs5+1:xe5+1, ye5+1:ye5+th+1]  # Obstacle 5
ccB[xs6:xe6, ye6+1:ye6+th+1] = cB[xs6+1:xe6+1, ye6+1:ye6+th+1]  # Obstacle 6
ccB[xs7:xe7, ye7+1:ye7+th+1] = cB[xs7+1:xe7+1, ye7+1:ye7+th+1]  # Obstacle 7
ccB[xs8:xe8, ye8+1:ye8+th+1] = cB[xs8+1:xe8+1, ye8+1:ye8+th+1]  # Obstacle 8
ccB[xs9:xe9, ye9+1:ye9+th+1] = cB[xs9+1:xe9+1, ye9+1:ye9+th+1]  # Obstacle 9
