...
def SpeciesBCs(phi, nx,ny, xs1,xe1,ys1,ye1, xs2,xe2,ys2,ye2, xs3,xe3,ys3,ye3, xs4,xe4,ys4,ye4, xs5,xe5,ys5,ye5, xs6,xe6,ys6,ye6):
...
@njit
def ReactionStep(cO2star, cBstar, dt, k, nx, ny):
    ...
    for i in range(1, nx+1):
        for j in range(1, ny+1):
            ...
            cO2[i,j] = cO2star[i,j] / (1 + dt*k*cBstar[i,j])
            cB[i,j] = cBstar[i,j] / (1 - dt*k*cO2[i,j])
...
def PlotFunctions(xx, yy, phi, x, y, X1, X2, X3, X4, X5, X6, Lx, Ly, title, x_label, y_label):
