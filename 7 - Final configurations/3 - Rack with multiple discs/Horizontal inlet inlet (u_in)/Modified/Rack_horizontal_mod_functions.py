...
def gammaCoeff(gamma, hx,hy, nout_start, nout_end, XX):
    ...
    def gamma_corners_obst(gamma, X, hx, hy, position):
        ...
        ''' if position == 'down':
            ...
            return gamma '''
        if position == 'down':
            xs = X[0]; xe = X[1]; ye = X[3]
            gamma[xs-1, ye-1] = hx*hy / (hx**2 + hy**2)
            gamma[xe+1, ye-1] = hx*hy / (hx**2 + hy**2)
            return gamma
        elif position == 'up':
            xs = X[0]; xe = X[1]; ys = X[2]
            gamma[xs-1, ys] = hx*hy / (hx**2 + hy**2)
            gamma[xe+1, ys] = hx*hy / (hx**2 + hy**2)
            return gamma
        elif position == 'south':
            xs = X[0]; ys = X[2]; xe_last = X[-3]
            gamma[xs-1, ys] = hx*hy / (hx**2 + hy**2)
    ...
    X1, X2_1, X3_1, X4_1, X5_1, X6_1, X7_1, X8_1, X9_1, X10_1, X11_1, X12_1, X13_1, X14_1, X15_1, X16_1, X17_1, X2_2, X3_2, X4_2, X5_2, X6_2, X7_2, X8_2,\
        X9_2, X10_2, X11_2, X12_2, X13_2, X14_2, X15_2, X16_2, X17_2, X18, X19, X20, X21, X22, X23, X24, X25, X26, X27, X28, X29, X30, X31, X32, X33 = XX
            
    xs1 = X1[0]; ys1 = X1[2]; ye1 = X1[3]
    gamma[xs1, ys1-1] = hx*hy / (hx**2 + hy**2)                # Obstacle 1
    gamma[xs1, ye1+1] = hx*hy / (hx**2 + hy**2)
    gamma = gamma_corners_obst(gamma, X2_2, hx, hy, 'down')    # Obstacle 2
    gamma = gamma_corners_obst(gamma, X3_1, hx, hy, 'up')      # Obstacle 3
    gamma = gamma_corners_obst(gamma, X4_2, hx, hy, 'down')    # Obstacle 4
    gamma = gamma_corners_obst(gamma, X5_1, hx, hy, 'up')      # Obstacle 5
    gamma = gamma_corners_obst(gamma, X6_2, hx, hy, 'down')    # Obstacle 6
    gamma = gamma_corners_obst(gamma, X7_1, hx, hy, 'up')      # Obstacle 7
    gamma = gamma_corners_obst(gamma, X8_2, hx, hy, 'down')    # Obstacle 8
    gamma = gamma_corners_obst(gamma, X9_1, hx, hy, 'up')      # Obstacle 9
    gamma = gamma_corners_obst(gamma, X10_2, hx, hy, 'down')   # Obstacle 10
    gamma = gamma_corners_obst(gamma, X11_1, hx, hy, 'up')     # Obstacle 11
    gamma = gamma_corners_obst(gamma, X12_2, hx, hy, 'down')   # Obstacle 12
    gamma = gamma_corners_obst(gamma, X13_1, hx, hy, 'up')     # Obstacle 13
    gamma = gamma_corners_obst(gamma, X14_2, hx, hy, 'down')   # Obstacle 14
    gamma = gamma_corners_obst(gamma, X15_1, hx, hy, 'up')     # Obstacle 15
    xs16_2 = X16_2[0]; ye16_2 = X16_2[3]
    gamma[xs16_2-1, ye16_2-1] = hx*hy / (hx**2 + hy**2)        # Obstacle 16
    xs17_1 = X17_1[0]; ys17_1 = X17_1[2]
    gamma[xs17_1-1, ys17_1] = hx*hy / (hx**2 + hy**2)          # Obstacle 17
...
def ImmobilizedCells(Xv, Xv0, X1, X2, th, position):
    
    xs1 = X1[0]; xe1 = X1[1]; ys1 = X1[2]; ye1 = X1[3]
    xs2 = X2[0]; xe2 = X2[1]; ys2 = X2[2]; ye2 = X2[3]

    if position == 'left':
        Xv[xs1-th:xs1, ys1:ye1] = Xv0
        Xv[xs2-th:xs2, ys2:ye2] = Xv0
    elif position == 'right':
        Xv[xe1:xe1+th, ys1:ye1] = Xv0
        Xv[xe2:xe2+th, ys2:ye2] = Xv0
...
@njit
def CellsMeanConcentration(cB, th, X1, X2, position):
    ...
    if position == 'left':
        for i in range(xs1-th, xs1):     # (pay attention if cells are above or under the plate!!!)
            for j in range(ys1, ye1+1):

                cBmean_o = cBmean
                cBmean = cBmean_o + cB[i,j]
        for i in range(xs2-th, xs2):
            for j in range(ys2, ye2+1):

                cBmean_o = cBmean
                cBmean = cBmean_o + cB[i,j]
    if position == 'right':
        for i in range(xe1, xe1+th):     # (pay attention if cells are above or under the plate!!!)
            for j in range(ys1, ye1+1):

                cBmean_o = cBmean
                cBmean = cBmean_o + cB[i,j]
        for i in range(xe2, xe2+th):
            for j in range(ys2, ye2+1):

                cBmean_o = cBmean
                cBmean = cBmean_o + cB[i,j]

    cBmean = cBmean / (((ye1- ys1) * th) + ((ye2 - ys2) * th))
...
def Interpolation_of_cells(ccXv, Xv, X1, X2, th, position):
    ...
    if position == 'left':
        ccXv[xs1-th:xs1, ys1:ye1] = Xv[xs1-th:xs1, ys1+1:ye1+1]
        ccXv[xs2-th:xs2, ys2:ye2] = Xv[xs2-th:xs2, ys2+1:ye2+1]
    elif position == 'right':
        ccXv[xe1:xe1+th, ys1:ye1] = Xv[xe1:xe1+th, ys1+1:ye1+1]
        ccXv[xe2:xe2+th, ys2:ye2] = Xv[xe2:xe2+th, ys2+1:ye2+1]
...
def MeanCellsPlot(t_vec, cBmean, Time, cB0):
    ...
    for i in range(len(cBmean)):
        plt.plot(t_vector,cBmean[i], label = f'Plate {i+2}')
