...
import pandas as pd
from   Alternate_horizontal_functions import *
from   Alternate_horizontal_obstacle_def import *
...
v_in_l = 5e-4      # m/s   (inlet lateral velocity)
...
nin_end1 = int(xs1/10)  # last cell index
'''# Inlet section lateral 2 (south side)
nin_start2 = xe5 + 1   # first cell index 
nin_end2 = xs6 - 1      # last cell index
# Inlet section lateral 3 (north side)
nin_start3 = xe8 + 1   # first cell index
nin_end3 = xs9 - 1      # last cell index'''

# Outlet section (east side)
nout_start = ye10 + 1   # first cell index (0 is the first one!!!)
nout_end = ny - 1       # last cell index

# Inlet/Outlet section areas and outlet velocity
Ain1 = hx*(nin_end1-nin_start1+1)       # inlet section area [m]
'''Ain2 = hx*(nin_end2-nin_start2+1)
Ain2 = hx*(nin_end3-nin_start3+1)
Ain = Ain1 + Ain2 + Ain3                # total inlet section area [m]'''
Aout = hy*(nout_end-nout_start+1)       # outlet section area [m]
u_out = (Ain1*v_in)/Aout
...
cBmean1 = cBmean2 = cBmean3 = cBmean4 = cBmean5 = cBmean6 = cBmean7 = cBmean8 = cBmean9 = cBmean10 = cB1out = t_vector = np.empty(0)
...
v[nin_start1-1:nin_end1+1, -1] = -v_in     # fixed inlet velocity (north)
u[-1, nout_start-1:nout_end+1] = u_out   # fixed outlet velocity (east)
'''v[:xs1, :] = -v_in/10                   # Internal points: fixed velocity [m/s]
v[xe1:xs2, :] = v_in/10; v[xe2:xs3, :] = - v_in/10; v[xe3:xs4, :] = v_in/10; v[xe4:xs5, :] =  - v_in/10
v[xe5:xs6, :] = v_in/10; v[xe6:xs7, :] = - v_in/10; v[xe7:xs8, :] = v_in/10; v[xe8:xs9, :] =  - v_in/10
u[xs10:, :] = v_in/10
v = v_initialize(v, XX) # set u = 0 over obstacles'''
vt = v
ut = u
...
for it in range(1, nsteps+1):
    ...
    v[nin_start1-1:nin_end1+1, -1] = -v_in     # fixed inlet velocity (north)
    '''v[nin_start2:nin_end2+1,  0] =  v_in_l   # fixed inlet velocity (south)
    v[nin_start3:nin_end3+1, -1] = -v_in_l   # fixed inlet velocity (north)'''
    ...
    vt[nin_start1-1:nin_end1+1, -1] = -v_in     # fixed inlet velocity (north)
    '''vt[nin_start2:nin_end2+1,  0] =  v_in_l   # fixed inlet velocity (south)
    ...
    Qin1  = abs(np.mean(v[nin_start1-1:nin_end1+1, -1])*Ain1)    # inlet flow rate [m2/s]
    '''Qin2  = abs(np.mean(v[nin_start2-1:nin_end2+1, 0])*Ain2)
    Qin3  = abs(np.mean(v[nin_start3-1:nin_end3+1, -1])*Ain3)
    Qin = Qin1 + Qin2 + Qin3'''
    Qout = np.mean(u[-1, nout_start-1:nout_end+1])*Aout    # outlet flow rate [m2/s]    
    if (abs(Qout)>1.e-6):
        u[-1, nout_start-1:nout_end+1] = u[-1, nout_start-1:nout_end+1] * abs(Qin1/Qout)
...
filename = 'Alternate_horizontal.dat'
...
ccXv = Interpolation_of_cells(ccXv, Xv, X1, th, cells_position)  # Obstacle 2
...
# Mean values of cB
MeanCellsPlot(t_vector, [cBmean1, cBmean2, cBmean3, cBmean4, cBmean5, cBmean6, cBmean7, cBmean8, cBmean9, cBmean10], Time, Xv0)
