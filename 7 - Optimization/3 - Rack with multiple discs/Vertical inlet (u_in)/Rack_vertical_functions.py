...
def gammaCoeff(gamma, hx,hy, nout_start, nout_end, XX):
    ...
    gamma = gamma_corners_obst(gamma, X18, hx, hy, 'up')     # Obstacle 18 (Look at the position!!!)
    gamma = gamma_corners_obst(gamma, X19, hx, hy, 'down')   # Obstacle 19
    gamma = gamma_corners_obst(gamma, X20, hx, hy, 'up')     # Obstacle 20
    gamma = gamma_corners_obst(gamma, X21, hx, hy, 'down')   # Obstacle 21
    gamma = gamma_corners_obst(gamma, X22, hx, hy, 'up')     # Obstacle 22
    gamma = gamma_corners_obst(gamma, X23, hx, hy, 'down')   # Obstacle 23
    gamma = gamma_corners_obst(gamma, X24, hx, hy, 'up')     # Obstacle 24
    gamma = gamma_corners_obst(gamma, X25, hx, hy, 'down')   # Obstacle 25
    gamma = gamma_corners_obst(gamma, X26, hx, hy, 'up')     # Obstacle 26
    gamma = gamma_corners_obst(gamma, X27, hx, hy, 'down')   # Obstacle 27
    gamma = gamma_corners_obst(gamma, X28, hx, hy, 'up')     # Obstacle 28
    gamma = gamma_corners_obst(gamma, X29, hx, hy, 'down')   # Obstacle 29
    gamma = gamma_corners_obst(gamma, X30, hx, hy, 'up')     # Obstacle 30
    gamma = gamma_corners_obst(gamma, X31, hx, hy, 'down')   # Obstacle 31
...
def PlotFunctions(xx, yy, phi, x, y, XX, Lx, Ly, title, x_label, y_label):
    ...
    fig, ax = plt.subplots()
    plot = plt.contourf(xx/Lx, yy/Ly, np.transpose(phi))
    plt.colorbar(plot)
    for i in range(len(XX)):
        add_obst_patch(XX[i])
    plt.title(title)
