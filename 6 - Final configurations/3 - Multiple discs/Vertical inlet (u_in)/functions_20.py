...
def gammaCoeff(gamma, hx,hy, nout_start,nout_end, XX):
    ...
    X1, X2, X3, X4, X5, X6, X7, X8, X9, X10, X11, X12, X13, X14, X15, X16, X17, X18, X19, X20, X21, X22, X23, X24, X25, X26, X27, X28, X29, X30, X31 = XX
...
def PlotFunctions(xx, yy, phi, x, y, XX, Lx, Ly, title, x_label, y_label):
    ...
    def add_obst_patch(X):
        ...
        for i in range(len(X) // 4):
            xs = X[4*i]; xe = X[4*i+1]; ys = X[4*i+2]; ye = X[4*i+3]
            ax.add_patch(Rectangle((x[xs-1], y[ys-1]), x[xe-xs+1], y[ye-ys+1], facecolor = 'grey'))
