...
def gammaCoeff(gamma, hx,hy, nout_start,nout_end, XX):
    ...
    gamma = gamma_corners_obst(gamma, X4, hx, hy, 'north')      # Obstacle 4
    gamma = gamma_corners_obst(gamma, X5, hx, hy, 'south')      # Obstacle 5
    gamma = gamma_corners_obst(gamma, X6, hx, hy, 'north')      # Obstacle 6
    gamma = gamma_corners_obst(gamma, X7, hx, hy, 'south')      # Obstacle 7
    gamma = gamma_corners_obst(gamma, X8, hx, hy, 'north')      # Obstacle 8
    gamma = gamma_corners_obst(gamma, X9, hx, hy, 'south')      # Obstacle 9
    xs10 = X10[0]; ye10 = X10[3]
...
@njit
def ReactionStep(cO2star, Xvstar, cGlcstar, cGlnstar, cLacstar, cAmmstar, dt, parameters, nx, ny):

    cO2 = cO2star; Xv = Xvstar; cGlc = cGlcstar
    cGln = cGlnstar; cLac = cLacstar; cAmm = cAmmstar

    mu_max, k_d, K_Glc, K_Gln, KI_Amm, KI_Lac, KD_Amm, KD_Lac, Y_Glc, Y_Gln, Y_Lac, Y_Amm = parameters
    ...
    return cO2, Xv, cGlc, cGln, cLac, cAmm
...
def Graphical_obstacles(uu, vv, pp, tau, ccO2, ccXv, ccGlc, ccGln, ccLac, ccAmm, XX):
    
    @njit
    def graphical(uu, vv, pp, tau, ccO2, ccXv, ccGlc, ccGln, ccLac, ccAmm, X):
        ...
        return uu, vv, pp, tau, ccO2, ccXv, ccGlc, ccGln, ccLac, ccAmm
        
    for i in range(len(XX)):
        uu, vv, pp, tau, ccO2, ccXv, ccGlc, ccGln, ccLac, ccAmm = graphical(uu, vv, pp, tau, ccO2, ccXv, ccGlc, ccGln, ccLac, ccAmm, XX[i])
